...
pion1 = ax1.scatter(tmp[0,0], tmp[0,1], color = "grey", marker = 'X', s = 100, label='Pioneer Pie')
pion2 = ax1.scatter(tmp[0,0], tmp[0,1], color = "red", marker = 'X', s = 100, label='Pioneer Pie')
...
swot4 = ax1.scatter(swot_pos[3,0], swot_pos[3,1], color = "black", marker = 'X', s = 100, label='SWOT Pie')
swot5 = ax1.scatter(swot_pos[4,0], swot_pos[4,1], color = "black", marker = 'X', s = 100, label='SWOT Pie')
...
def update(frame):
    ...
    # Deployed
    if frame > findframe(gps = pos, point = pioneer_pos[0,:]):
        w = pioneer_pos[0,0]
        z = pioneer_pos[0,1]
        # update the scatter plot:
        data = np.stack([w, z]).T
    ...
    if frame > findframe(gps = pos, point = pioneer_pos[1,:]):
        w = pioneer_pos[1,0]
        z = pioneer_pos[1,1]
        # update the scatter plot:
        data = np.stack([w, z]).T
    ...
    # Retreaved
    if frame > findframe(gps = pos, point = swot_pos[0,:]):
        w = tmp[0,0]
        z = tmp[0,0]
        # update the scatter plot:
        data = np.stack([w, z]).T
    ...
    if frame > findframe(gps = pos, point = swot_pos[1,:]):
        w = tmp[0,0]
        z = tmp[0,0]
        # update the scatter plot:
    ...
    if frame > findframe(gps = pos, point = swot_pos[2,:]):
        w = tmp[0,0]
        z = tmp[0,0]
        # update the scatter plot:
    ...
    if frame > findframe(gps = pos, point = swot_pos[3,:]):
        w = tmp[0,0]
        z = tmp[0,0]
        # update the scatter plot:
        data3 = np.stack([w, z]).T
        swot4.set_offsets(data3)

    if frame > findframe(gps = pos, point = swot_pos[4,:]):
        w = tmp[0,0]
        z = tmp[0,0]
        # update the scatter plot:
        data3 = np.stack([w, z]).T
        swot5.set_offsets(data3)
